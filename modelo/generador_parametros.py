...
import pandas as pd
import numpy as np
from modelo.asignador_capacidad import AsignadorCapacidad
...
def __generar_costos_cif_cargas(parametros: dict, conjuntos: dict, file=str):
    ...
    transitos_a_puerto_df.rename(
        columns={'valor_kg': 'valor_cif'}, inplace=True)

    inventarios_puerto_df.rename(
        columns={'valor_cif_kg': 'valor_cif'}, inplace=True)
...
def __capacidad_almacenamiento_planta(parametros: dict, conjuntos: dict, file: str):
    ...
    asignador_capacidad = AsignadorCapacidad(file=file)

    inventario_planta_df = asignador_capacidad.obtener_unidades_almacenamiento()
    ...
    capacidad_df = inventario_planta_df.groupby(['planta', 'ingrediente_actual'])[
        conjuntos['ingredientes']].sum().reset_index()

    capacidad_df['capacidad'] = capacidad_df.apply(
...
def __inventario_planta(parametros: dict, conjuntos: dict, file: str):
    ...
    inventario_planta_df = pd.read_excel(
        file, sheet_name='unidades_almacenamiento', usecols='A:D')
...
def __consumo_proyectado(parametros: dict, conjuntos: dict, file: str):
    ...
    mean_df.set_index(['ingrediente', 'planta'], drop=True, inplace=True)
    ...
    demanda_df = demanda_df.melt(
        id_vars=['ingrediente', 'planta'], var_name='fecha', value_name='consumo')
...
def generar_parametros(parametros: dict, conjuntos: dict, file: str) -> dict:
    ...
    __capacidad_recepcion_ingredientes(
        parametros=parametros, conjuntos=conjuntos, file=file)
    ...
    __max_cantidad_camiones_a_despachar(
        parametros=parametros, conjuntos=conjuntos)
